...
def CreateMatrixForSvd(Y,sampleLen,nSample,maxLen, singleSampleDim):
  ...
  for index,seq in enumerate(Y):
    lenght=sampleLen[index]
    #print lenght
    for j,timestep in enumerate(seq):
      
      #inserisco il time step su tutta la sua diagonale
      for i in range(0,lenght):
	data.extend(timestep)
	rows.extend([sRow+i]*singleSampleDim)
	cols.extend(range(singleSampleDim*i,singleSampleDim*(i+1)))
      #ora passo al prossimo timestep, qindi ora dovro cominciare a scrivere alla riga successiva
      sRow=sRow+1
      #il timestpe corrente comparira in xhi una volta in meno rispetto al precedente
      lenght=lenght-1
    #ora sRow punta alla riga successiva alla somma delle lunghezza degli esempi scritti fino ad ora
    #e quindi posso inserire alllo stesso modo la successiva sequenza
  #ho scritto tutte le sequenze, ora posso creare la matrice sparsa
  #avra un numero di righe pari alla somma delle lunghezze delle sequenze, ed il numero di colonne sara la massima lunghzza moltiplcata per la dimensione del singolo timestep
...
def accEval(model,epoch,ftr,fts,fvl):
  gt=np.load('./TrainingSet.txt.npy')
  ds=loadDataSetMin(trainingSet)
...
if __name__ == "__main__":
  ...
  #raw_input()
  _,  sampleLen,  nSample, maxLen = LoadDataForPreTraining()

  loadDataset=np.load('./TrainingSet.txt.npy')
  
  print type(loadDataset)

  #DEBUG
  #loadDataset=np.array([loadDataset[0]])
  #END DEBUG
  dataset=[]
  for seq in loadDataset:
    dataset.append(seq)
  ...
  print "compute SVD"
  (v,s,u_t)=SvdForBigData(M,InputDim,nHidden)
  ...
  model = MetaRNN(n_in=InputDim, n_out=88, nHidden=nHidden,
  ...
  hiddenWeights=GetWeightsForHidnOut(loadDataset,targets,model,nHidden)
  ...
  #TRAINING
  xt1 = time.strftime('%s')
  model.fit(loadDataset, targets, validation_frequency=1000)
  xt2 = time.strftime('%s')
